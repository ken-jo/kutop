"""Optional, domain-specific kubetop plugins + the generic plugin seam.

The kubetop CORE (``fetch.py`` + ``render/app.py``) is workload-agnostic. Some
...
_REGISTRY: "Optional[list[KubetopPlugin]]" = None

# Module paths of the built-in optional plugins. Each must expose ``PLUGIN``
# (a KubetopPlugin instance). Adding a plugin = appending its module path here.
_BUILTIN_PLUGIN_MODULES = (
    "kubetop.plugins.health",
)


def _discover() -> "list[KubetopPlugin]":
...
def iter_plugins() -> "list[KubetopPlugin]":
...
def iter_enabled(config: Any) -> "Iterable[KubetopPlugin]":
